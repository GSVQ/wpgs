				...
				<article id="comment-<?php comment_ID(); ?>" class="comment">
					<footer>
						...
						<?php if ( $comment->comment_approved == '0' ) : ?>
							<em class="awaiting-moderation"><?php _e( 'Tu comentario está esperando a ser moderado.', 'vantage' ); ?></em>
						<?php endif; ?>
					</footer>
...
function vantage_posted_on() {
	$posted_on_parts = array(
		'on' => __('<a href="%1$s" title="%2$s" rel="bookmark"><time class="entry-date" datetime="%3$s">%4$s</time></a>', 'vantage'),
		'by' => __( '<span class="byline"> | Por <span class="author vcard"><a class="url fn n" href="%5$s" title="%6$s" rel="author">%7$s</a></span></span>', 'vantage' ),
	);
	...
	$posted_on = sprintf( implode(' ', $posted_on_parts),
		...
		esc_attr( sprintf( __( 'Ver todos los artículos de %s', 'vantage' ), get_the_author() ) ),
		get_the_author()
	);
...
function vantage_get_post_categories(){
	...
	if ( ! vantage_categorized_blog() ) {
		...
		if ( '' != $tag_list ) {
			$meta_text = __( 'Subido en %1$s and tagged %2$s.', 'vantage' );
		}
		else {
			$meta_text = __( 'Subido a %1$s.', 'vantage' );
		}
